...
if [ ! -f .env ]; then sudo -u "$APP_USER" cp .env.example .env; fi
sudo cp systemd/printstream.service /etc/systemd/system/printstream.service
sudo systemctl daemon-reload
echo "Installation abgeschlossen. Jetzt .env bearbeiten und dann: sudo systemctl enable --now printstream"
